...
(function($){
	...
	_zTreeTools = function(setting, zTreeTools) {
		zTreeTools.getAllChecked = function() {
			...
			var node = nodes[i];
				var checkState = node.check_Child_State;
		...
		zTreeTools.getAllParentChecked = function() {
			var nodes = this.getCheckedNodes(true);
			var length = nodes.length;
			var results = [];
			var keys = [];
			...
			for(var i = 0 ; i < length ;i ++){
				...
				var checkState = node.check_Child_State;
				if(checkState ===-1 ){
					var pNode = this.getPnode(node);
					if(pNode.check_Child_State === 2){
						if($.inArray(pNode.id, keys) === -1){
							keys.push(pNode.id);
							results.push(pNode);
						}
					}else{
