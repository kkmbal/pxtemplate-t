...
public class FileDownloadUtil {
   ...
   * @throws IOException
   */
  public static void download(HttpServletRequest request, HttpServletResponse response, File file, String orgName)
    ...
    //path 조작 체크.
	private static void pathCheck(File file) throws IOException {
		String[] saveDirs = null;
		String[] upDirs = null;
		
		try{
		    String saveDir = PortalxpertConfigUtils.getString("upload.real.dir");
		    saveDir = saveDir.replace("/", File.separator);
		    saveDirs = saveDir.split("\\"+File.separator);
		    upDirs = file.getPath().replace("..\\","").replace("../","").split("\\"+File.separator);
		}catch(Exception e){
			e.printStackTrace();
		}
	    
		for(int i=0;i<2;i++){
			if(!saveDirs[i].equals(upDirs[i])){
				throw new IOException("다운로드 불가능한 위치입니다.");
			}
		}
		
	    //if(saveDirs != null && upDirs != null && !saveDirs[0].equals(upDirs[0]) && !saveDirs[1].equals(upDirs[1])){
	    //	throw new IOException("다운로드 불가능한 위치입니다.");
	    //}
	}
  ...
  public static void download(HttpServletRequest request, HttpServletResponse response, InputStream is,
      String filename, long filesize, String mimetype, String orgName) throws ServletException, IOException {
    ...
    if (userAgent.indexOf("MSIE 5.5") > -1) { // MS IE 5.5 이하
      //response.setHeader("Content-Disposition", "filename=" + URLEncoder.encode(filename, "UTF-8") + ";");
    	...
    	if(CommUtil.isDevMode()) {	//개발
    		response.setHeader("Content-Disposition", "attachment; filename="+ java.net.URLEncoder.encode(orgName, "UTF-8") + ";");
    	}else {		//서버
    		response.setHeader("Content-Disposition", "attachment; filename="+ java.net.URLEncoder.encode(orgName, "UTF-8").replaceAll("\\+", " ") + ";");
    	}
    } else { // 모질라나 오페라
